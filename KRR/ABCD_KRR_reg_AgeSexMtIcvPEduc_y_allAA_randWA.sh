#!/bin/sh
#
# Wrapper script to perform kernel ridge regression only on all AA + randomly selected WA 
# (i.e. 50% AA, 50% WA).
#
# Jingwei Li, 20200826
...
NAME:
    ABCD_KRR_reg_AgeSexMtIcvPEduc_y_allAA_randWA.sh

DESCRIPTION:
    Wrapper script to perform kernel ridge regression only on all AA + randomly selected WA 
    (i.e. 50% AA, 50% WA).
    It calls ABCD_KRR_in_subgroup.sh to first extract functional connectivity of the subset 
    of subjects (i.e. all AA + random WA), then run kernel ridge regression only within these
    subjects.
...
ARGUMENTS:
    ...
    -split_dir     <split_dir>    : Full path of the directory containing the split folds of the subpopulation
                                    (i.e. all AA + randomly selected WA).
                                    It is the output directory of ../match_split/ABCD_create_subfold_allAA_randWA.m
                                    Default: 
                                    ${GREEN}$split_dir${NOCOLOR}
    -split_fstem   <split_fstem>  : A string shared across all filenames of split folds. It should be the same 
                                    string as passed into ../match_split/ABCD_create_subfold_allAA_randWA.m
