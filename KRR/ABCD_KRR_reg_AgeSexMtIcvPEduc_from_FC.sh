...
# Confounding variables are regressed from resting-state functional connectivity.
#
# Jingwei Li, 20200721
...
main() {
    ...
    for b in $behaviors; do
	    ...
	    cmd="$DIR/ABCD_KRR.sh -bhvr_name $b -subj_ls $subj_ls -cfds_ls $cfds_ls \
...
ARGUMENTS:
    ...
    -FC_file       <FC_file>      : Full path of a .mat file containing a 3D matrix which is the resting-state
                                    functional connectivity of all subjects. Confounding variables should have
                                    been regressed out from the matrix.
                                    It is the output file of ../ABCD_regress_cfds_from_FC.m. Default:
                                    $FC_file
...
EXAMPLE:
    ...
    -csvname <path_to_lists>/phenotypes_pass_rs.txt -bhvr_ls <path_to_lists>/behavior_list.txt -subj_ls \\
    <path_to_lists>/subjects_pass_rs_pass_pheno.txt -FC_file \\
    <path_to_RSFC>/pass_rs_pass_pheno_5351_reg_AgeSexMtIcvPEduc.mat -subfold_dir /.../matchANDsplit/ \\
    -subfold_stem _pass_rs_pass_pheno -outdir /.../reg_AgeSexMtIcvPEduc_fr_FC/
" 1>&2; exit 1; }
