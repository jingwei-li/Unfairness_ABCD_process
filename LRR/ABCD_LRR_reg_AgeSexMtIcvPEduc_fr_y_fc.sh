#!/bin/sh
#
# Wrapper script to perform linear ridge regression on whole population.
...
outdir=$proj_dir/models/LRR/20210629/reg_AgeSexMtIcvPEduc_fr_y_FC
...
main() {
	...
	for b in $behaviors; do
		...
		cmd="$DIR/ABCD_LRR.sh -bhvr_name $b -subj_ls $subj_ls -subfold_f $subfold_f -FC_file \
$FC_file -outdir $outdir/$b -outstem $b -csvname $csvname -cfds_ls $cfds_ls -cfds_X_ls $cfds_X_ls"

		jname=ABCD_LRR_${b}
		$CBIG_CODE_DIR/setup/CBIG_pbsubmit -cmd "$cmd" -walltime 65:00:00 -mem ${memory}G \
...
usage() { echo "
NAME:
    ABCD_LRR_reg_AgeSexMtIcvPEduc_fr_y_FC.sh
...
EXAMPLE:
    $DIR/ABCD_LRR_reg_AgeSexMtIcvPEduc_fr_y_FC.sh \\
